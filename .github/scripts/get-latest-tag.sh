...
LATEST_TAG=$(gcloud artifacts docker images list --sort-by=~CREATE_TIME --include-tags --format='value(TAGS)' --filter="tags:*" --limit=1 "$REGION-docker.pkg.dev/$PROJECT_ID/$REPOSITORY_NAME/$ENVIRONMENT-api-image" || echo "default")
